...
const main = () => {
  const node = createStruct(new Node(250,300), 8); 
  let target;
  const stack = [node];
...
const csvtojson = csv => {
  ...
  const result = lines.map(line => {
    const words = line.split(','); //["140", "34.875767", "128.626294", "139", "-1", "", "", ""]
    const [id, y, x, ...branch] = words.map(word => Number(word)) // [140, 34.875767, 128.626294, 139, -1, 0, 0, 0]
  ...
  for(let i = 0; i < lines.length; i++){
    const words = lines[i].split(','); //["140", "34.875767", "128.626294", "139", "-1", "", "", ""]
    const [id, y, x, ...branch] = words.map(word => Number(word)) // [140, 34.875767, 128.626294, 139, -1, 0, 0, 0]
    arr.push({id, y, x, branch: branch.slice(0, branch.indexOf(-1))});
  }  
  return result;
}
